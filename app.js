...
// v4.0.2 custom dialog fallback
function snhCustomExitDialog(){
  return snhCustomExitDialog("Exit Sri Nidhi Study Hall?\n\nDo you want to exit the app?");
}
...
window.deleteStudent=id=>{const s=db.students.find(x=>x.id===id);if(!s)return;if(snhCustomExitDialog(`Delete ${s.name}? Fee, attendance and movement history will remain in reports.`)){db.students=db.students.filter(x=>x.id!==id);logAction('student_deleted',`${s.id} - ${s.name} deleted`);saveDB();drawStudents()}};
...
window.deleteFee=id=>{if(snhCustomExitDialog('Delete payment?')){db.fees=db.fees.filter(f=>f.id!==id);saveDB();renderFees()}};
...
window.resetDemo=()=>{if(snhCustomExitDialog('Reset all data?')){db=clone(defaultDB);saveDB();render('dashboard')}};
...
window.deleteNotice=id=>{if(snhCustomExitDialog('Delete notice?')){db.notices=db.notices.filter(x=>x.id!==id);saveDB();renderNotices()}}
...
window.deleteFee=id=>{const f=db.fees.find(x=>x.id===id);if(!f)return;if(snhCustomExitDialog(`Delete receipt ${f.receipt||''}?`)){db.fees=db.fees.filter(x=>x.id!==id);logAction('fee_deleted',`Deleted ${f.receipt||id}`);saveDB();drawFees()}};
...
function renderMovement(){
 ...
 el('moveForm').onsubmit=async e=>{
   ...
   try{
     ...
     const continueWithout=snhCustomExitDialog(`GPS location రాలేదు: ${err.message}\n\nGPS లేకుండా exit record save చేయాలా?`);
...
window.markReturned=async id=>{
 ...
 if(!snhCustomExitDialog(`Mark ${studentName(x.studentId)} as returned?`))return;
...
window.addEventListener('popstate',function snhExitGuard(e){
  ...
  const leave=window.snhCustomExitDialog('Exit Sri Nidhi Study Hall?\n\nDo you want to exit the app?');
