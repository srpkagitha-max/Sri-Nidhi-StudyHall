...
function v29InRange(date,from,to){const d=String(date||'').slice(0,10);return !!d&&d>=from&&d<=to}
...
function v29ReportFilters(){return {type:el('reportType')?.value||'attendance',from:v29DateValue(el('reportFrom')?.value,v29StartOfMonth()),to:v29DateValue(el('reportTo')?.value,v29EndOfMonth()),studentId:el('reportStudent')?.value||'',status:el('reportStatus')?.value||''}}
...
function renderReports(){
 ...
 el('pageContent').innerHTML=`
 <section class="reports-hero"><div><p>V2.9 REPORTING CENTRE</p><h1>Reports & Exports</h1><span>Attendance, fees, entry/exit and student-wise records in one place.</span></div><div class="reports-version">v${V29_VERSION}</div></section>
 ...
 <div class="card"><div class="card-heading-row"><div><h3 id="reportHeading">Report Preview</h3><p class="muted" id="reportMeta"></p></div></div><div id="reportTable"></div></div>`;
 el('reportType').value=type;
 ['reportType','reportFrom','reportTo','reportStudent','reportStatus'].forEach(id=>el(id).onchange=()=>{if(id==='reportType'){window._v29Type=el(id).value;const s=el('reportStatus');s.innerHTML=v29StatusOptions(el(id).value)}v29GenerateReport()});
 v29GenerateReport();
}
function v29AttendanceRows(f){return db.attendance.filter(a=>v29InRange(a.date,f.from,f.to)&&(!f.studentId||a.studentId===f.studentId)&&(!f.status||a.status===f.status)).sort((a,b)=>String(b.date).localeCompare(String(a.date))||studentName(a.studentId).localeCompare(studentName(b.studentId))).map(a=>[a.date,a.studentId,studentName(a.studentId),a.status,a.time||''])}
function v29FeeRows(f){return db.fees.filter(x=>v29InRange(x.date,f.from,f.to)&&(!f.studentId||x.studentId===f.studentId)).sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(x=>{const due=Number(x.amount||0),paid=Number(x.paid||0),status=paid>=due?'Paid':'Partial';return [x.date,x.studentId,studentName(x.studentId),x.month||'',due,paid,Math.max(0,due-paid),x.mode||'',x.receipt||'',status]}).filter(r=>!f.status||r[9]===f.status)}
function v29MovementRows(f){return db.movements.filter(x=>v29InRange(x.outTime,f.from,f.to)&&(!f.studentId||x.studentId===f.studentId)&&(!f.status||x.status===f.status)).sort((a,b)=>String(b.outTime).localeCompare(String(a.outTime))).map(x=>[String(x.outTime||'').replace('T',' '),x.studentId,studentName(x.studentId),x.destination||'',x.reason||'',String(x.expectedReturn||'').replace('T',' '),String(x.returnTime||'').replace('T',' '),v29DurationMinutes(x.outTime,x.returnTime),Array.isArray(x.locationHistory)?x.locationHistory.length:0,x.status])}
function v29StudentRows(f){return db.students.filter(s=>(!f.studentId||s.id===f.studentId)&&(!f.status||s.status===f.status)).map(s=>{const att=db.attendance.filter(a=>a.studentId===s.id&&v29InRange(a.date,f.from,f.to)),fees=db.fees.filter(x=>x.studentId===s.id&&v29InRange(x.date,f.from,f.to)),mov=db.movements.filter(x=>x.studentId===s.id&&v29InRange(x.outTime,f.from,f.to));return [s.id,s.name,s.course||'',s.batch||'',s.status||'Active',att.filter(a=>a.status==='Present').length,att.filter(a=>a.status==='Absent').length,att.filter(a=>a.status==='Late').length,fees.reduce((n,x)=>n+Number(x.paid||0),0),mov.length]})}
...
window.v29GenerateReport=()=>{const {f,title,headers,rows}=v29Data();window._v29From=f.from;window._v29To=f.to;window._v29Type=f.type;el('reportHeading').textContent=title;el('reportMeta').textContent=`${f.from} to ${f.to} • ${rows.length} record${rows.length===1?'':'s'}`;el('reportTable').innerHTML=rows.length?`<div class="table-wrap report-table"><table><thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${r.map((c,i)=>`<td>${(f.type==='fees'&&[4,5,6].includes(i))||(f.type==='student'&&i===8)?money(c):esc(c===null||c===undefined?'':c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`:'<div class="empty">No report records found for the selected filters.</div>';v29RenderKpis(f)}
function v29RenderKpis(f){const att=db.attendance.filter(a=>v29InRange(a.date,f.from,f.to)),fees=db.fees.filter(x=>v29InRange(x.date,f.from,f.to)),mov=db.movements.filter(x=>v29InRange(x.outTime,f.from,f.to));const cards=[['Attendance Marked',att.length],['Present',att.filter(a=>a.status==='Present').length],['Fees Collected',money(fees.reduce((n,x)=>n+Number(x.paid||0),0))],['Outside Trips',mov.length],['Returned',mov.filter(x=>x.status==='Returned').length]];el('reportKpis').innerHTML=cards.map(([l,v])=>`<div class="report-kpi"><span>${l}</span><b>${v}</b></div>`).join('')}
window.v29ExportCSV=()=>{const {title,headers,rows}=v29Data();v28DownloadCSV(`${title.replace(/[^a-z0-9]+/gi,'-')}-${today()}.csv`,[headers,...rows])}
window.v29ExportPDF=()=>{const {f,title,headers,rows}=v29Data();if(!window.jspdf)return alert('PDF library loading. Please try again.');const {jsPDF}=window.jspdf,doc=new jsPDF({orientation:headers.length>6?'landscape':'portrait'}),w=doc.internal.pageSize.getWidth();doc.setFontSize(16);doc.text(db.settings.hallName,14,16);doc.setFontSize(12);doc.text(title,14,24);doc.setFontSize(9);doc.text(`${f.from} to ${f.to} | Records: ${rows.length}`,14,31);let y=40;const line=(r,head=false)=>{const text=r.map((c,i)=>`${head?headers[i]+': ':''}${String(c??'')}`).join(' | ');const wrapped=doc.splitTextToSize(text,w-28);if(y+wrapped.length*5>doc.internal.pageSize.getHeight()-12){doc.addPage();y=16}doc.setFont(undefined,head?'bold':'normal');doc.text(wrapped,14,y);y+=wrapped.length*5+3};if(!rows.length)doc.text('No records found.',14,y);else rows.forEach(r=>line(r,true));doc.save(`${title.replace(/[^a-z0-9]+/gi,'-')}-${f.from}-to-${f.to}.pdf`)}
