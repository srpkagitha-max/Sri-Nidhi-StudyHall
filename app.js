...
function renderMovement(){
 ...
 <div class="card movement-form-card"><div class="card-heading-row"><div><h3>Student Entry / Exit</h3><p class="muted">Record a student going outside and return time.</p></div><span class="version-chip">v2.8</span></div>
 <form id="moveForm" class="form-grid"><div class="field"><label>Student</label><select name="studentId" required>${studentOptions()}</select></div><div class="field"><label>Out Time</label><input type="datetime-local" name="outTime" value="${v28DateTimeLocal()}" required></div><div class="field"><label>Expected Return</label><input type="datetime-local" name="expectedReturn"></div><div class="field span-2"><label>Reason / Place</label><input name="reason" placeholder="Reason and place" required></div><div class="field"><label>Contact / Note</label><input name="contactNote" placeholder="Optional"></div><div class="span-3"><button class="primary">Save Exit Record</button></div></form></div>
 <div class="card"><div class="card-heading-row"><div><h3>Currently Outside</h3><p class="muted">Return students with one tap.</p></div></div><div id="outsideList"></div></div>
 <div class="card"><div class="card-heading-row"><div><h3>Movement History</h3><p class="muted">Search and export entry / exit records.</p></div></div><div class="movement-filters"><input id="movementSearch" placeholder="Search student / reason"><input id="movementDate" type="date"><select id="movementStatus"><option value="">All Status</option><option>Outside</option><option>Returned</option></select><button class="secondary" onclick="exportMovementCSV()">Export CSV</button></div><div id="movementHistory"></div></div>`;
 el('moveForm').onsubmit=e=>{e.preventDefault();const o=Object.fromEntries(new FormData(e.target));if(db.movements.some(m=>m.studentId===o.studentId&&m.status==='Outside'))return alert('This student is already marked outside.');o.id=uid();o.status='Outside';o.returnTime='';db.movements.push(o);logAction('went_out',`${studentName(o.studentId)} went outside`);saveDB();renderMovement()};
 el('movementSearch').oninput=drawMovementHistory;el('movementDate').onchange=drawMovementHistory;el('movementStatus').onchange=drawMovementHistory;drawOutsideList();drawMovementHistory();
}
...
function renderMovement(){
 ...
 el('pageContent').innerHTML=`
 <div class="movement-stats"><div><small>Currently Outside</small><b>${outside.length}</b></div><div><small>Overdue</small><b>${overdue.length}</b></div><div><small>Returned Today</small><b>${returnedToday}</b></div><div><small>Total Records</small><b>${all.length}</b></div></div>
 <div class="card movement-form-card"><div class="card-heading-row"><div><h3>Student Entry / Exit</h3><p class="muted">Exit save చేసినప్పుడు ఈ device GPS tracking start అవుతుంది.</p></div><div class="tracking-head"><span class="tracking-status stopped">🔴 Stopped</span><span class="version-chip">v2.8.2 Stable</span></div></div>
