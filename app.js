...
const STUDENT_SESSION_KEY='sriNidhiStudentSessionV1';
function digitsOnly(v){return String(v||'').replace(/\D/g,'')}
function phoneMatches(student,input){const n=digitsOnly(input).slice(-10);if(n.length<10)return false;return [student.phone,student.parentPhone,student.emergencyPhone].some(v=>digitsOnly(v).slice(-10)===n)}
function switchLoginRole(role){
 const student=role==='student';el('adminLoginPanel').classList.toggle('hidden',student);el('studentLoginPanel').classList.toggle('hidden',!student);
 el('showAdminLogin').classList.toggle('active',!student);el('showStudentLogin').classList.toggle('active',student);
...
function renderStudentPage(page='overview'){
 ...
 const a=studentAttendanceStats(s),f=studentFeeStats(s),mov=[...db.movements].filter(x=>x.studentId===s.id).sort((x,y)=>String(y.outTime).localeCompare(String(x.outTime)));
...
const savedStudent=activeStudent();if(savedStudent)setTimeout(()=>openStudentPortal(savedStudent.id),0);
...
renderMovement=function(){
 const list=[...db.movements].sort((a,b)=>String(b.outTime).localeCompare(String(a.outTime))),outside=list.filter(x=>x.status==='Outside'),returned=list.filter(x=>x.status!=='Outside');
 el('pageContent').innerHTML=`<div class="attendance-stats"><div><small>Currently Outside</small><b>${outside.length}</b></div><div><small>Total Requests</small><b>${list.length}</b></div></div><div class="card"><h3>Students Currently Outside</h3><p class="muted">Name paina press chesthe complete details, latest real GPS location, 5 updates mariyu call buttons kanipisthayi.</p><div class="admin-outside-list">${outside.length?outside.map(x=>adminMovementCard(x)).join(''):'<div class="empty">No students are outside now.</div>'}</div></div><div class="card"><h3>Returned / Previous Requests</h3><div class="admin-outside-list">${returned.length?returned.map(x=>adminMovementCard(x)).join(''):'<div class="empty">No previous permission records.</div>'}</div></div>`;
};
...
adminMovementCard=function(x){
 ...
 const latest=points.length?points[points.length-1]:(x.lastLocation||x.startLocation);
...
const badge=[...document.querySelectorAll('body>div')].find(x=>x.textContent&&x.textContent.includes('v3.3 RC'));if(badge)badge.textContent='v4.0.2 • Custom Exit Dialog';
...
function snhBackState(mode,type='guard'){
  return mode==='student'
    ? (type==='root'?{snhRoot:'student',studentPage:'overview'}:{snhGuard:'student',studentPage:'overview'})
    : (type==='root'?{snhRoot:'admin',page:'dashboard'}:{snhGuard:'admin',page:'dashboard'});
}
function snhInitBackGuard(mode){
  snhBackMode=mode;
  snhExitInProgress=false;
  snhExitDialogOpen=false;
  snhCloseExitDialog(false);
  history.replaceState(snhBackState(mode,'root'),'',location.href);
  history.pushState(snhBackState(mode,'guard'),'',location.href);
}
...
function snhEnsureExitDialog(){
  let dialog=el('snhExitDialog');
  if(dialog)return dialog;
  dialog=document.createElement('div');
  dialog.id='snhExitDialog';
  dialog.className='snh-exit-overlay hidden';
  dialog.setAttribute('role','dialog');
  dialog.setAttribute('aria-modal','true');
  dialog.setAttribute('aria-labelledby','snhExitTitle');
  dialog.innerHTML=`<section class="snh-exit-card">
    <div class="snh-exit-logo"><img src="icon-192.png" alt=""></div>
    <h2 id="snhExitTitle">Exit Sri Nidhi Study Hall?</h2>
    <p>Do you want to exit the app?</p>
    <div class="snh-exit-actions">
      <button type="button" class="snh-stay-btn" id="snhStayBtn">Stay</button>
      <button type="button" class="snh-exit-btn" id="snhExitBtn">Exit</button>
    </div>
  </section>`;
  document.body.appendChild(dialog);
  dialog.addEventListener('click',event=>{if(event.target===dialog)snhChooseExit(false)});
  el('snhStayBtn').addEventListener('click',()=>snhChooseExit(false));
  el('snhExitBtn').addEventListener('click',()=>snhChooseExit(true));
  return dialog;
}
function snhOpenExitDialog(mode){
  if(snhExitDialogOpen)return;
  snhExitDialogOpen=true;
  snhBackMode=mode;
  const dialog=snhEnsureExitDialog();
  dialog.classList.remove('hidden');
  document.body.classList.add('snh-dialog-open');
  setTimeout(()=>el('snhStayBtn')?.focus(),20);
}
function snhCloseExitDialog(restoreFocus=true){
  const dialog=el('snhExitDialog');
  if(dialog)dialog.classList.add('hidden');
  document.body.classList.remove('snh-dialog-open');
  snhExitDialogOpen=false;
  if(restoreFocus)setTimeout(()=>document.querySelector('#appView:not(.hidden) button, #studentAppView:not(.hidden) button')?.focus(),0);
}
function snhChooseExit(leave){
  if(!snhExitDialogOpen)return;
  const mode=snhBackMode;
  snhCloseExitDialog(false);
  if(leave){
    snhExitInProgress=true;
    // Dialog opens after restoring the guard entry. Two steps leave the app safely.
    setTimeout(()=>history.go(-2),40);
  }
}
window.addEventListener('keydown',event=>{
  if(!snhExitDialogOpen)return;
  if(event.key==='Escape'){event.preventDefault();snhChooseExit(false)}
});
window.addEventListener('popstate',function snhExitGuard(e){
  ...
  // Restore the guard immediately so the app cannot close behind the custom dialog.
  history.pushState(snhBackState(mode,'guard'),'',location.href);
  if(snhExitDialogOpen)return;
  snhOpenExitDialog(mode);
});
